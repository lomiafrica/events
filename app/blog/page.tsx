...
const urlFor = (source: any) => ({
  image: () => ({
    url: () =>
      source?.asset?._ref ||
      "https://via.placeholder.com/600x400.png?text=Card+Image",
  }),
  width: () => urlFor(source),
          ...
          {posts && posts.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 xl:gap-10">
              {posts.map((post) => (
                <Card
                  key={post._id}
                  className="overflow-hidden flex flex-col h-full bg-card text-card-foreground shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-lg border border-border"
                >
                  {post.image && (
                    <Link
                  ...
                  <CardFooter className="p-5 sm:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-t border-border">
                    ...
                    <Button
                      variant="outline"
                      asChild
                      className="mt-auto sm:mt-0 w-full sm:w-auto hover:bg-primary hover:text-primary-foreground transition-colors duration-200 border-primary/50 text-primary"
                    >
                      <Link href={`/blog/${post.slug.current}`}>
                        Read More &rarr;
                      </Link>
