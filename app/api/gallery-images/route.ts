...
interface CloudinaryError {
    error?: { // Make properties optional as structure might vary
        message: string;
...
export async function GET() {
    ...
    console.log("[API Route] Cloudinary Folder Env:", process.env.CLOUDINARY_FOLDER);
    console.log("[API Route] Cloudinary Cloud Name:", process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME);

    if (!process.env.CLOUDINARY_FOLDER || !process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME) {
         console.error("[API Route] Missing required environment variables.");
         return NextResponse.json({ error: "Server configuration error: Missing Cloudinary environment variables." }, { status: 500 });
    }
    ...
    try {
        ...
        console.log(`[API Route] Fetched ${results?.resources?.length ?? 0} resources from Cloudinary.`);
        ...
        const imagePropsPromises = resources.map(async (result: CloudinaryResource, i: number): Promise<ImageProps | null> => {
            ...
            if (isNaN(height) || isNaN(width)) {
                 console.warn(`[API Route] Invalid dimensions for ${result.public_id}: height=${result.height}, width=${result.width}. Skipping image.`);
        ...
        const imagesWithoutBlur: (ImageProps | null)[] = await Promise.all(imagePropsPromises);
        // Filter out nulls (due to invalid dimensions)
        const finalImages: ImageProps[] = imagesWithoutBlur.filter((img): img is ImageProps => img !== null);

        console.log(`[API Route] Processed ${finalImages.length} images successfully (without blur).`);
        return NextResponse.json(finalImages);

    } catch (error: unknown) {
        console.error("[API Route] Error fetching or processing Cloudinary data:", error);
        ...
        return NextResponse.json({ error: "Internal Server Error fetching gallery images." }, { status: 500 });
    }
} 