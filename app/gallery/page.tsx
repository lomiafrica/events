...
export default function GalleryPage() {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    try {
                        ...
                        console.error("[Gallery Client] Failed to parse error response:", parseError);
                    }
                ...
                if (!Array.isArray(fetchedImages)) {
                    console.error("[Gallery Client] API response is not an array:", fetchedImages);
                    throw new Error("Invalid data format received from server.");
                }

                console.log(`[Gallery Client] Successfully fetched ${fetchedImages.length} images.`);
    ...
    const zoomedImage = zoomedImageId !== null ? images.find(img => img.id === zoomedImageId) : null;
                        ...
                        return (
                            ...
                            >
                                <Image
                                    ...
                                    // Use parsed numbers, provide fallbacks if parsing fails
            ...
            {zoomedImage && (
                ...
                >
                    {(() => { // Use IIFE to calculate classes based on zoomedImage
                        ...
                        const isValidDimensions = !isNaN(numericWidth) && !isNaN(numericHeight);
                        const isLandscape = isValidDimensions && numericWidth > numericHeight;

                        // Determine container size based on orientation
                        const containerMaxWidth = isLandscape ? 'md:max-w-4xl' : 'md:max-w-xl'; // Larger for landscape
                        ...
                        return (
                            ...
                            >
                                <Image
                                    ...
                                    height={baseHeight}
                                    sizes={imageSizes}
                                // Consider adding priority={true} here if landscape LCP is an issue
                                />
                                {/* Close button: Visible only on mobile (hidden on md and up) */}
                                <button
                                    onClick={handleCloseModal} // Use handler to close
                                    className="absolute top-2 right-2 p-2 rounded-md bg-black/50 text-white hover:bg-black/75 transition-colors md:hidden z-10" // Added z-10
