...
export const getTranslations = (lang: Language): Record<string, TranslationValue> => {
  return translations[lang] || translations.en;
};

export const t = (lang: Language, key: string, values?: Record<string, string | number | undefined>) => {
  try {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (currentVal && typeof currentVal === "object" && k in currentVal) {
        ...
        if (i === keys.length - 1 && (typeof currentVal === "string" || typeof currentVal === "number")) {
          const currentValStr = String(currentVal);
          // Perform interpolation if values are provided
          if (values) {
            return currentValStr.replace(/\{(\w+)\}|\[\[(\w+)\]\]/g, (match, p1, p2) => {
        ...
        console.warn(`Translation missing for key part: ${k} in key: ${key}, lang: ${lang}. Falling back.`);
