export function initImageScroller() {
  setTimeout(() => {
    ...
    function navigateToImage(newIndex) {
      if (newIndex >= 0 && newIndex < thumbnails.length && newIndex !== currentImageIndex) {
    ...
    function handleScroll(e) {
      ...
      scrollTimeout = setTimeout(() => {
        ...
        const clampedIndex = Math.max(0, Math.min(newIndex, thumbnails.length - 1));
    ...
    function handleKeyDown(e) {
      if (document.activeElement === imageScroller || imageScroller.contains(document.activeElement)) {
