  ...
  setTimeout(() => {
    ...
    function handleTouchMove(e) {
      ...
      if (Math.abs(deltaX) > Math.abs(deltaY)) {
        ...
        if (deltaX > 0) {
          // Swipe left
          const nextImage =
            document.querySelector(".li.active").nextElementSibling;
          if (nextImage) {
          ...
          // Swipe right
          const prevImage =
            document.querySelector(".li.active").previousElementSibling;
          if (prevImage) {
    ...
    thumbnails.forEach((thumbnail) => {
      thumbnail.addEventListener("click", () => {
        ...
        thumbnail.parentElement.scrollIntoView({
          behavior: "smooth",
          inline: "center",
          block: "nearest",
        });
        ...
        if (thumbnail.parentElement) {
          ...
          console.error("Thumbnail lacks parentElement:", thumbnail);
        }
      });
    });
    ...
    thumbnails.forEach((thumbnail) =>
      observer.observe(thumbnail.parentElement),
    );
  }, 0);
}
