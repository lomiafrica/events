...
if (supabaseUrl && supabaseAnonKey) {
    try {
        ...
        console.error("Failed to initialize Supabase client in CheckoutButton:", error);
    }
} else {
    console.warn("Supabase URL or Anon Key is missing from .env. API checkout (modal) will be disabled if these are not set.");
}
...
export default function CheckoutButton({
    ...
    const availabilityStatus = getItemAvailabilityStatus(
        { active: item.active, stock: item.stock, salesStart: item.salesStart, salesEnd: item.salesEnd },
        currentLanguage
    );
    ...
    if (globallyTicketsOnSale && availabilityStatus.available) {
        ...
        return (
            <Badge
                ...
                className="text-sm sm:w-auto justify-center py-2 px-3 border-slate-600 text-slate-400 rounded-sm h-10 inline-flex items-center"
            >
                {availabilityStatus.reason || t(currentLanguage, "eventSlugPage.availability.unavailable")}
            </Badge>
        );
    }
} 