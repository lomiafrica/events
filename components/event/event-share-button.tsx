...
export function EventShareButton({
  ...
  const baseButtonClasses =
    "flex items-center justify-start gap-2 text-left h-12 rounded-xs text-white transition-colors duration-200";
  ...
  return (
    <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-[425px] p-4 rounded-xs border border-border/40 bg-background backdrop-blur-sm">
          <DialogHeader>
          ...
          <DialogFooter className="flex flex-col sm:flex-row sm:justify-between gap-2 -mt-2">
            <div className="flex items-center space-x-2 flex-1">
              ...
              <Button
                ...
                variant="outline"
                onClick={handleCopy}
                className="rounded-xs hover:bg-accent/30"
                disabled={!shareUrl}
              >
                {copied ? (
