...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: ticketData, error: verifyError } = await supabase.rpc(
      ...
      const isLegacyTicket = ticket.use_count !== undefined && ticket.use_count !== null && ticket.total_quantity !== undefined;
      ...
      if (auto_admit && canBeAdmitted) {
        ...
        if (admitResult === 'SUCCESS') {
          ...
          ticketWithRemaining.remaining_tickets = Math.max(0, ticketWithRemaining.remaining_tickets - 1);
